...
const Sidebar = ({ children }) => {
  ...
  const sidebarlinks = [
    ...
    
    {
      ...
      title:'Softwares',
      link:'/admin/dashboard/softwares',
      index:2
    },  
    {
...
Please Open this on Desktop or Laptop
    ...
    <div className=" hidden sm:hidden md:flex">
      ...
      <div className={` min-h-screen fixed ${togglebanner?'top-[0px]':'top-[180px]'} left-0 bg-[var(--primary-color)] flex flex-col justify-between ${ toggleside? 'w-[73px]':'w-[200px]'} pr-5`} >
        ...
        <div title="Toggle Sidebar" className={`absolute -right-8 -rotate-90 top-[57px] bg-white rounded-full cursor-pointer ${ !toggleside && 'rotate-90'}`} onClick={()=>setToggleSide(!toggleside)}>
          ...
          <div key={index} onClick={()=>setActiveState(index)} title={ele.disable && 'Wait For Next Version'} className={`mt-3 pr-0 ${ele.disable && 'cursor-no-drop'} ${ !ele.disable && ' hover:bg-[var(--secondary-color)] from-[#42B2BC] to-[#1E5156] hover:shadow-md'} flex gap-4 group py-2 pl-3 rounded-r-full cursor-pointer items-center ${activeState == index && !ele.disable? "bg-[var(--secondary-color)]":""}`}>
